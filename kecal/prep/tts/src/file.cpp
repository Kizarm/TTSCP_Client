...
bool WavOutput::header(const char * data, const int len) {
  ...
  info.format   = SF_FORMAT_WAV | SF_FORMAT_PCM_16; // compress or SF_FORMAT_PCM_16;
...
static short amplify (const short in) { // hlas violky je poněkud slabý
  int a = in * 3;
  const int max = 32000;
  if (a > +max) return +max;            // ale jsou tam špičky, nutno ořezat
  if (a < -max) return -max;
  return a;
}
bool WavOutput::write(const char * data, const int len) {
  ...
  for (int n=0; n<(len<<1); n++) {
    tmpbuf[n] = amplify(ptr[n]);
  }
  sf_write_short (outfile, tmpbuf, len >> 1);
  return true;
}
